...
----------------------------------
CREATE INDEX IF NOT EXISTS idx_mv_ocs2d_fid ON ocs2d.ocs2d_hdf_2005_2010_2015_2021_2024 USING BTREE (fid);
CREATE INDEX IF NOT EXISTS idx_mv_ocs2d_cs21 ON ocs2d.ocs2d_hdf_2005_2010_2015_2021_2024 USING BTREE (cs24);
CREATE INDEX IF NOT EXISTS idx_mv_ocs2d_us21 ON ocs2d.ocs2d_hdf_2005_2010_2015_2021_2024 USING BTREE (us24);
CREATE INDEX IF NOT EXISTS idx_mv_ocs2d_cs21 ON ocs2d.ocs2d_hdf_2005_2010_2015_2021_2024 USING BTREE (cs21);
CREATE INDEX IF NOT EXISTS idx_mv_ocs2d_us21 ON ocs2d.ocs2d_hdf_2005_2010_2015_2021_2024 USING BTREE (us21);
CREATE INDEX IF NOT EXISTS idx_mv_ocs2d_cs15 ON ocs2d.ocs2d_hdf_2005_2010_2015_2021_2024 USING BTREE (cs15);
CREATE INDEX IF NOT EXISTS idx_mv_ocs2d_us15 ON ocs2d.ocs2d_hdf_2005_2010_2015_2021_2024 USING BTREE (us15);
CREATE INDEX IF NOT EXISTS idx_mv_ocs2d_cs10 ON ocs2d.ocs2d_hdf_2005_2010_2015_2021_2024 USING BTREE (cs10);
CREATE INDEX IF NOT EXISTS idx_mv_ocs2d_us10 ON ocs2d.ocs2d_hdf_2005_2010_2015_2021_2024 USING BTREE (us10);
CREATE INDEX IF NOT EXISTS idx_mv_ocs2d_cs05 ON ocs2d.ocs2d_hdf_2005_2010_2015_2021_2024 USING BTREE (cs05);
CREATE INDEX IF NOT EXISTS idx_mv_ocs2d_us05 ON ocs2d.ocs2d_hdf_2005_2010_2015_2021_2024 USING BTREE (us05);
CREATE INDEX IF NOT EXISTS dx_mv_ocs2d_geom ON ocs2d.ocs2d_hdf_2005_2010_2015_2021_2024 USING GIST (geom);

-----------------------------------------
--- Génération de l'enveloppe urbaine ---
-----------------------------------------
/*
 * Génération de l'enveloppe urbaine pour les millésimes 2005, 2010, 2015, 2021 et 2024
 ...
 */
 --- Début Millésime 2024 ---

--- Phase 1 : dilatation et érosion à 50m
DROP TABLE IF EXISTS wip.ocs2d_enveloppe_urbaine_2024 CASCADE;
CREATE TABLE wip.ocs2d_enveloppe_urbaine_2024 AS
WITH dilatation_enveloppe AS (
    ...
    FROM
        ocs2d.ocs2d_hdf_2005_2010_2015_2021_2024 AS ocs 
    WHERE
...
UPDATE wip.ocs2d_enveloppe_urbaine_2024 AS t
SET geom = a.geom
FROM (
    ...
    FROM (
        ...
        FROM wip.ocs2d_enveloppe_urbaine_2024
...
WITH fragments AS (
    ...
    FROM
        wip.ocs2d_enveloppe_urbaine_2024
),
...
UPDATE wip.ocs2d_enveloppe_urbaine_2024
...
ALTER TABLE wip.ocs2d_enveloppe_urbaine_2024
ADD PRIMARY KEY (gid);

-- Création d'un index spatial sur la colonne geom
CREATE INDEX idx_ocs2d_enveloppe_urbaine_2024_geom 
ON wip.ocs2d_enveloppe_urbaine_2024 
USING GIST (geom);
...
-- Tableau des emprises agricoles pour le millésime 2024
-- Cette requête utilise la classe d'occupation du sol (OCS) pour extraire les emprises agricoles
-- Les classes US1.1, US1.1.7 et US1.4.0 sont sélectionnées pour l'année 2024
-- Les polygones sont unis et la géométrie est corrigée en utilisant la fonction ST_Buffer
-- Un index spatial est créé sur la colonne geom
DROP TABLE IF EXISTS wip.structure_agricole_2024;
CREATE TABLE wip.structure_agricole_2024 AS
  SELECT
      (ST_Dump(ST_Buffer(ST_Union(ocs.geom), 0.0))).geom::geometry(Polygon, 2154) AS geom
  FROM
      ocs2d.ocs2d_hdf_2005_2010_2015_2021_2024 AS ocs
  WHERE
      (LEFT(ocs.us24, 5) = 'US1.1' AND LEFT(ocs.cs24, 5) = 'CS1.1') OR (LEFT(ocs.us24, 7) = 'US1.1.7' AND NOT LEFT(ocs.cs24, 7) IN ('CS3.1.2', 'CS3.2.1', 'CS3.2.2')) OR (LEFT(ocs.us24, 7) = 'US1.4.0' AND LEFT(ocs.cs24, 5) = 'CS1.1')
  ;
...
-- Cette requête permet de générer une couche de résultat de la matrice ENAF pour les millésimes 2005, 2010, 2015, 2021 et 2024
...
-- - ocs2d.ocs2d_hdf_2005_2010_2015_2021_2024 : table contenant les données de l'occupation du sol pour les millésimes 2005, 2010, 2015, 2021 et 2024
...
DROP TABLE IF EXISTS ocs2d.ocs2d_enaf_2005_2010_2015_2021;
CREATE TABLE ocs2d.ocs2d_enaf_2005_2010_2015_2021 AS
...
--- Analyse du bati agricole 2010 ---
  ...
  ocs.fid,
    --------------------------------
  -- Ventilation ENAF pour 2024 --
  --------------------------------
  CASE
    -- Ventilation selon la localisation par rapport à l'enveloppe urbaine
    WHEN ((LEFT(us24, 5) = 'US1.1' AND LEFT(cs24, 5) = 'CS1.1') OR (LEFT(us24, 7) = 'US1.1.7' AND NOT LEFT(cs24,7) IN ('CS3.1.2','CS3.2.1','CS3.2.2')) OR (LEFT(ocs.us24, 7) = 'US1.4.0' AND LEFT(ocs.cs24, 5) = 'CS1.1')) AND ba_2024.contexte IS TRUE THEN 'Activité'
    WHEN ((LEFT(us24, 5) = 'US1.1' AND LEFT(cs24, 5) = 'CS1.1') OR (LEFT(us24, 7) = 'US1.1.7' AND NOT LEFT(cs24,7) IN ('CS3.1.2','CS3.2.1','CS3.2.2')) OR (LEFT(ocs.us24, 7) = 'US1.4.0' AND LEFT(ocs.cs24, 5) = 'CS1.1')) AND ba_2024.contexte IS FALSE THEN 'Agricole'
    -- Ventilation selon la nomenclature en 4 postes
    WHEN LEFT(us24,7) = 'US7.0.0' AND LEFT(cs24,7) IN ('CS1.2.1','CS5.1.1','CS6.6.0') AND p4_24 = 1 THEN 'Urbanisé autre'
    WHEN LEFT(us24,7) = 'US7.0.0' AND LEFT(cs24,7) IN ('CS1.2.1','CS5.1.1','CS6.6.0') AND p4_24 = 2 THEN 'Agricole'
    WHEN LEFT(us24,7) = 'US7.0.0' AND LEFT(cs24,7) IN ('CS1.2.1','CS5.1.1','CS6.6.0') AND p4_24 = 3 THEN 'Naturel'
    WHEN LEFT(us24,7) = 'US7.0.0' AND LEFT(cs24,7) IN ('CS1.2.1','CS5.1.1','CS6.6.0') AND p4_24 = 4 THEN 'Infrastructure'
    -- Ventilation selon matrice sans pondération 4 postes ou enveloppe urbaine 
    WHEN (LEFT(us24,5) = 'US1.1' 
      OR LEFT(us24,5) = 'US1.4' 
      OR LEFT(us24,7) IN ('US2.1.3','US2.1.5')) 
      AND NOT LEFT(cs24,7) IN ('CS3.1.2','CS3.2.1','CS3.2.2') THEN 'Agricole'
    WHEN (LEFT(us24,5) = 'US1.2' AND NOT LEFT(cs24,7) IN ('CS3.1.2','CS3.2.1','CS3.2.2')) 
      OR (LEFT(us24,7) IN ('US6.2.3','US7.0.0') AND LEFT(cs24,3) = 'CS4') THEN 'Forêt'
    WHEN (LEFT(us24,7) IN ('US2.1.1','US2.1.2','US2.1.4','US2.2.0') 
      OR LEFT(us24,5) = 'US3.1'
      OR LEFT(us24,7) IN ('US3.2.2','US3.2.3','US3.2.4','US3.2.5')
      OR (LEFT(us24,7) = 'US3.2.1' AND LEFT(cs24,3) = 'CS1')
      OR LEFT(us24,7) = 'US6.2.1')
      AND NOT LEFT(cs24,7) IN ('CS3.1.2','CS3.2.1','CS3.2.2') THEN 'Activité'
    WHEN LEFT(us24,3) = 'US5' AND NOT LEFT(cs24,7) IN ('CS3.1.2','CS3.2.1','CS3.2.2') THEN 'Habitat'
    WHEN (LEFT(us24,3) = 'US4' AND LEFT(cs24,3) IN ('CS1','CS2','CS4','CS5','CS6')) 
      OR (LEFT(us24,3) = 'US4' AND LEFT(cs24,7) IN ('CS3.1.1')) THEN 'Infrastructure'
    WHEN (LEFT(us24,7) IN ('US6.1.1','US6.2.2') AND NOT LEFT(cs24,7) IN ('CS3.1.2','CS3.2.1','CS3.2.2'))
      OR (LEFT(us24,7) = 'US7.0.0' AND LEFT(cs24,7) IN ('CS1.1.1','CS1.1.2','CS1.2.2')) THEN 'Urbanisé autre'
    WHEN LEFT(us24,7) IN ('US1.3.1','US1.3.2','US6.1.2')
      OR (LEFT(us24,7) = 'US3.2.1' AND LEFT(cs24,3) IN ('CS2','CS3','CS4','CS5','CS6'))
      OR (LEFT(us24,7) = 'US4.4.0' AND LEFT(cs24,7) = 'CS3.1.1')
      OR (LEFT(us24,7) = 'US6.2.3' AND LEFT(cs24,3) IN ('CS1','CS2','CS3','CS5','CS6'))
      OR (LEFT(us24,7) = 'US7.0.0' AND (LEFT(cs24,7) = 'CS1.2.1' OR LEFT(cs24,3) IN ('CS2','CS3','CS5','CS6')))
      OR LEFT(cs24,7) IN ('CS3.1.2','CS3.2.1','CS3.2.2') THEN 'Naturel'
    ELSE 'XX' -- Contrôle en cas de non association
  END AS enaf_2024,
  CASE
    -- Ventilation selon la localisation par rapport à l'enveloppe urbaine
    WHEN ((LEFT(us24, 5) = 'US1.1' AND LEFT(cs24, 5) = 'CS1.1') OR (LEFT(us24, 7) = 'US1.1.7' AND NOT LEFT(cs24,7) IN ('CS3.1.2','CS3.2.1','CS3.2.2')) OR (LEFT(ocs.us24, 7) = 'US1.4.0' AND LEFT(ocs.cs24, 5) = 'CS1.1')) AND ba_2024.contexte IS TRUE THEN 'Non-NAF'
    WHEN ((LEFT(us24, 5) = 'US1.1' AND LEFT(cs24, 5) = 'CS1.1') OR (LEFT(us24, 7) = 'US1.1.7' AND NOT LEFT(cs24,7) IN ('CS3.1.2','CS3.2.1','CS3.2.2')) OR (LEFT(ocs.us24, 7) = 'US1.4.0' AND LEFT(ocs.cs24, 5) = 'CS1.1')) AND ba_2024.contexte IS FALSE THEN 'NAF'
    -- Ventilation selon la nomenclature en 4 postes
    WHEN LEFT(us24,7) = 'US7.0.0' AND LEFT(cs24,7) IN ('CS1.2.1','CS5.1.1','CS6.6.0') AND p4_24 = 1 THEN 'Non-NAF'
    WHEN LEFT(us24,7) = 'US7.0.0' AND LEFT(cs24,7) IN ('CS1.2.1','CS5.1.1','CS6.6.0') AND p4_24 = 2 THEN 'NAF'
    WHEN LEFT(us24,7) = 'US7.0.0' AND LEFT(cs24,7) IN ('CS1.2.1','CS5.1.1','CS6.6.0') AND p4_24 = 3 THEN 'NAF'
    WHEN LEFT(us24,7) = 'US7.0.0' AND LEFT(cs24,7) IN ('CS1.2.1','CS5.1.1','CS6.6.0') AND p4_24 = 4 THEN 'Non-NAF'
    -- Ventilation selon matrice sans pondération 4 postes ou enveloppe urbaine 
    WHEN (LEFT(us24,5) = 'US1.1' 
      OR LEFT(us24,5) = 'US1.4' 
      OR LEFT(us24,7) IN ('US2.1.3','US2.1.5')) 
      AND NOT LEFT(cs24,7) IN ('CS3.1.2','CS3.2.1','CS3.2.2') THEN 'NAF'
    WHEN (LEFT(us24,5) = 'US1.2' AND NOT LEFT(cs24,7) IN ('CS3.1.2','CS3.2.1','CS3.2.2')) 
      OR (LEFT(us24,7) IN ('US6.2.3','US7.0.0') AND LEFT(cs24,3) = 'CS4') THEN 'NAF'
    WHEN (LEFT(us24,7) IN ('US2.1.1','US2.1.2','US2.1.4','US2.2.0') 
      OR LEFT(us24,5) = 'US3.1'
      OR LEFT(us24,7) IN ('US3.2.2','US3.2.3','US3.2.4','US3.2.5')
      OR (LEFT(us24,7) = 'US3.2.1' AND LEFT(cs24,3) = 'CS1')
      OR LEFT(us24,7) = 'US6.2.1')
      AND NOT LEFT(cs24,7) IN ('CS3.1.2','CS3.2.1','CS3.2.2') THEN 'Non-NAF'
    WHEN LEFT(us24,3) = 'US5' AND NOT LEFT(cs24,7) IN ('CS3.1.2','CS3.2.1','CS3.2.2') THEN 'Non-NAF'
    WHEN (LEFT(us24,3) = 'US4' AND LEFT(cs24,3) IN ('CS1','CS2','CS4','CS5','CS6')) 
      OR (LEFT(us24,3) = 'US4' AND LEFT(cs24,7) IN ('CS3.1.1')) THEN 'Non-NAF'
    WHEN (LEFT(us24,7) IN ('US6.1.1','US6.2.2') AND NOT LEFT(cs24,7) IN ('CS3.1.2','CS3.2.1','CS3.2.2'))
      OR (LEFT(us24,7) = 'US7.0.0' AND LEFT(cs24,7) IN ('CS1.1.1','CS1.1.2','CS1.2.2')) THEN 'Non-NAF'
    WHEN LEFT(us24,7) IN ('US1.3.1','US1.3.2','US6.1.2')
      OR (LEFT(us24,7) = 'US3.2.1' AND LEFT(cs24,3) IN ('CS2','CS3','CS4','CS5','CS6'))
      OR (LEFT(us24,7) = 'US4.4.0' AND LEFT(cs24,7) = 'CS3.1.1')
      OR (LEFT(us24,7) = 'US6.2.3' AND LEFT(cs24,3) IN ('CS1','CS2','CS3','CS5','CS6'))
      OR (LEFT(us24,7) = 'US7.0.0' AND (LEFT(cs24,7) = 'CS1.2.1' OR LEFT(cs24,3) IN ('CS2','CS3','CS5','CS6')))
      OR LEFT(cs24,7) IN ('CS3.1.2','CS3.2.1','CS3.2.2') THEN 'NAF'
    ELSE 'XX' -- Contrôle en cas de non association
  END AS urba_2024,
  ...
  CASE
    ...
    ELSE 'XX' -- Contrôle en cas de non association
  END AS urba_2005,
  -- Aide au debug de la classification ENAF 2024
  cs24,
  us24,
  p4_24,
...
FROM
  ocs2d.ocs2d_hdf_2005_2010_2015_2021_2024 AS ocs
LEFT JOIN bati_agricole_2024 AS ba_2024 ON ocs.fid = ba_2024.fid -- sélection des infrastructures agricoles 2024 
